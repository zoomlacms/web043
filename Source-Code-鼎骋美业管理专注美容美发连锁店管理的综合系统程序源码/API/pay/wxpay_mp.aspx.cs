...
namespace ZoomLaCMS.API.pay
{
    ...
    public partial class wxpay_mp : System.Web.UI.Page
    {
        ...
        //public string SuccessUrl { get { return Request.QueryString["SuccessUrl"] ?? "/Extend/Mobile/UserOrder.aspx"; } }
        public string SuccessUrl = "/Extend/Mobile/UserOrder.aspx";
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (payMod.Status != (int)M_Payment.PayStatus.NoPay) { function.WriteErrMsg("该订单不可付款"); }
                M_OrderList orderMod = orderBll.SelModelByOrderNo(payMod.PaymentNum.Trim(','));
                if (orderMod.StoreID < 1) { function.WriteErrMsg("订单未绑定店铺"); }
                M_CommonData storeMod = conBll.SelReturnModel(orderMod.StoreID);
                if (storeMod == null) { function.WriteErrMsg("店铺信息不存在"); }
                appMod = wxBll.SelReturnModel(DataConverter.CLng(storeMod.SpecialID));
                if (appMod == null) { function.WriteErrMsg("店铺未绑定微信"); }
