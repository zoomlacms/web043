<%@ Page Language="C#" AutoEventWireup="true" CodeFile="EditWxMenu.aspx.cs" Inherits="ZoomLaCMS.Manage.WeiXin.EditWxMenu"  MasterPageFile="~/Manage/I/Default.master" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    ...
    <script>
        ...
        var wxmenu = {};
        wxmenu.initMenu = function () {
            if (!confirm("确定要初始化吗,现有的数据将会丢失")) { return false; }
            var id = "<%:StoreId%>";
            var initBtns = '[{"type":"view","name":"商城","url":"https://7esy.z01.com/Extend/Mobile/StoreIndex.aspx?id=' + id + '","sub_button":[]},{"type":"view","name":"发现","url":"https://7esy.z01.com/Extend/Mobile/Find.aspx?id=' + id + '","sub_button":[]},{"type":"view","name":"我","url":"https://7esy.z01.com/Extend/Mobile/UserInfo.aspx?id=' + id + '","sub_button":[]}]';
            menudata = JSON.parse(initBtns);
            SaveMenu();
            location = location;
        }
        //=======================
            ...
            if (menudata.length > 0) {
                ...
                $.post("EditWxMenu.aspx?action=create&appid=<%:AppId%>", { menus: JSON.stringify(menudata) }, function (data) {
                    ...
                    if (APIResult.isok(model)) { alert('保存成功!'); }
                        ...
                        var ret = JSON.parse(model.retmsg);
                        if (ret.errmsg == "ok") { alert('保存成功!'); } else { alert("保存出错!"+model.retmsg) }
                    }
                })
            } else {
                alert("请添加菜单!");
