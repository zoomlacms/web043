<%@ Page Language="C#" AutoEventWireup="true" CodeFile="StoreUpdate.aspx.cs" Inherits="ZoomLaCMS.Manage.UserShopMannger.StoreUpdate" ValidateRequest="false" MasterPageFile="~/Manage/I/Default.master" %>
...
<asp:Content runat="server" ContentPlaceHolderID="Content">
    <div>
        <table class="table table-striped table-bordered table-hover">
            ...
            <tr><td class="text-right">店长：</td><td>
               ...
               <tr><td class="text-right">绑定微信：</td>
                <td>
                    <asp:DropDownList runat="server" ID="BindWX_DP" DataTextField="Alias" DataValueField="ID" class="form-control text_300"></asp:DropDownList>
                    <a href="javascript:;" onclick="go('WxConfig.aspx?id={id}');" class="btn btn-info btn-sm">前往配置</a>
                    <a href="javascript:;" onclick="go('EditWxMenu.aspx?appid={id}');" class="btn btn-info btn-sm">修改菜单</a>
                    <script>
                        function go(name) {
                            var id = $("#BindWX_DP").val();
                            if (id == "未绑定微信") { alert("请先选定微信号"); return; }
                            name = name.replace("{id}", id);
                            var url = "<%:customPath2%>WeiXin/" + name;
                            window.open(url);
                        }
                    </script>
                </td>
