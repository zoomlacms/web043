...
<script>
    ...
    function ajaxlogin() {
        if (!loginflag) {
            ...
            $("#IbtnEnter").val('登录中..');
            var param = { action: 'login', user: $("#<%=AdminN_T.ClientID%>").val(), pwd: RSAEncrypt.encrypt($("#<%=AdminP_T.ClientID%>").val()), VCode_hid: $("#VCode_hid").val(), vcode: $("#VCode").val(), zncode: $("#ZnCode_T").val() };
